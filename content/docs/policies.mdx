...
| Pack | Use case |
|------|----------|
| `sre.yaml` | SRE / DevOps agents — destructive ops require approval, prod-tagged resources locked down |
| `vibe-coder.yaml` | AI coding assistants — block secret commits, gate force-push, scan dependency installs |
| `hobbyist.yaml` | Solo builders — secret egress + obvious-bad URL blocks; lightweight defaults |
| `browser-agent.yaml` | Browser Use / Computer Use agents — block card/SSN form submits, threat-intel match on URLs |
| `enterprise-oem.yaml` | OEM / platform integrators — strict tenant isolation, audit-heavy, opt-in by feature |
